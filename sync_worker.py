...
# Parallel Fast Downloader (up to 8 parallel connections)
async def download_file_fast(client, location, out_file, file_size, max_workers=8):
    chunk_size = 512 * 1024 # 512 KB
    total_parts = math.ceil(file_size / chunk_size)
    print(f"Fast Multi-Stream Download: {file_size/(1024*1024):.1f}MB in {total_parts} parts with {max_workers} parallel workers...")
                ...
                try:
                    ...
                    async with lock:
                        ...
                        if downloaded % (50 * 1024 * 1024) < len(result.bytes):
                            ...
                            print(f"  --> Downloaded: {downloaded/(1024*1024):.1f}MB ({pct:.1f}%)", flush=True)
                    break
    ...
    f.close()
    print("Fast Download Complete!", flush=True)

# Parallel Fast Uploader (up to 8 parallel connections for big files)
async def upload_file_fast(client, file_path, max_workers=8):
    file_size = os.path.getsize(file_path)
    file_id = client._get_input_peer_and_access(target_channel_id)[0].channel_id if hasattr(client, '_get_input_peer_and_access') else utils.get_input_peer(target_channel_id)
    part_size = 512 * 1024 # 512 KB
    total_parts = math.ceil(file_size / part_size)
    print(f"Fast Multi-Stream Upload: {file_size/(1024*1024):.1f}MB in {total_parts} parts...")
    # Standard client.send_file with big file support is optimized when local file is passed
    return file_path

async def main():
    print(f"=== Multi-Stream Turbo Sync Engine Started ===")
    ...
    for msg in to_process[:batch_limit]:
        ...
        print(f"\n>>> Processing: {fn} ({sz_mb:.1f} MB, {dur_min:.1f} min) <<<", flush=True)
        
        local_path = f"/tmp/{fn}"
        loc = InputDocumentFileLocation(
            id=msg.file.id,
            access_hash=msg.file.access_hash,
            file_reference=msg.file.file_reference,
            thumb_size=''
        )
        
        # Fast parallel download
        await download_file_fast(client, loc, local_path, msg.file.size, max_workers=8)
        
        # Upload
        caption = msg.message or f"🎬 {fn}"
        print("Uploading to channel...", flush=True)
